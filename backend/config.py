...
class Config:
    ...
    def _get_database_url(explicit, cloud, user, password, host, port, dbname, cloud_sql_conn):
        ...
        if cloud_sql_conn:
            ...
            # When running on Cloud Run with `--add-cloudsql-instances`, the
            # recommended libpq-compatible form is to put the unix socket path
            # in the `host` query parameter. psycopg2/libpq understands URLs
            # like: postgresql://user:pass@/dbname?host=/cloudsql/PROJECT:REGION:INSTANCE
    ...
    # Tesseract OCR
    # Default to a common Windows install path; can be overridden by env
